...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(
            f"{str(Path.home())}/.Flux/logs/server_{datetime.now().strftime('%d-%m-%Y_%H-%M-%S')}.log"
        ),
            ...
            match request["type"]:
                ...
                case HeaderCode.UPDATE_HASH:
                    ...
                    if username is not None:
                        ...
                        try:
                            ...
                            user_share = Flux_db.search(User.uname == username)[0]["share"]
                            ...
                            Flux_db.update({"share": user_share}, User.uname == username)
                        except IndexError:
                            raise RequestException(
                                msg=f"Username does not exist",
                                code=ExceptionCode.NOT_FOUND,
                ...
                case HeaderCode.FILE_BROWSE:
                    ...
                    if username is not None and user_exists:
                        ...
                        browse_files: list[DBData] = Flux_db.search(User.uname == request["query"].decode(FMT))
