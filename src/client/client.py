...
# Set the client's IP and server's IP, address
...
share_directory = project_root / "share"
recv_directory = project_root / "downloads"
sub_directory = share_directory / "sub"

for directory in [log_directory, share_directory, recv_directory, sub_directory]:
                ...
                case "1":
                    ...
                    if response_header_type == HeaderCode.FILE_SEARCH.value:
                        ...
                        if len(browse_files):
                            ...
                            # print(f"Selected file: {file_choice}")
                            file_item = find_file(selected_user["share"], file_choice)

                            # print(f"received file: {file_item}")
...
if __name__ == "__main__":
    try:
        ...
        try:
            with open(f"/Flux/db/{my_username}_transfer_progress.obj", mode="rb") as transfer_dump:
        ...
        with open(f"/Flux/db/{my_username}_transfer_progress.obj", mode="wb") as transfer_dump:
